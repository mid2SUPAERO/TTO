...
class IPOPT_Problem:
    ...
    def calc_reduction_pattern_mapped(self):
        ...
        if self.myBCs.isReduced:
            reduction_pattern_map = np.zeros(
                self.myBCs.ncel_str * self.myBCs.N_cell_max
            )
            for i in range(
                self.n_topologies_cell
            ):  # starting from the lower left corner
                reduction_pattern_map += np.kron(
                    self.cell_mapping_matrix[:, i], self.myBCs.reduction_pattern[i, :]
                )
    ...
    def calc_area_phys(self, area):
        ...
        if self.myBCs.isReduced:
            ...
            for i in range(
                ...
                area_phys += np.kron(
                    self.cell_mapping_matrix[:, i],
                    area[i * self.N_cell[0] : (i + 1) * self.N_cell[0]],
                )  # initialize a_phys
                a_cell_out[i] = np.array(
                    area[i * self.N_cell[0] : (i + 1) * self.N_cell[0]]
                )
    ...
    def calc_area_phys_id(self):
        ...
        if self.myBCs.isReduced:
            ...
            for i in range(
                ...
                tmp += np.kron(
                    self.cell_mapping_matrix[:, i], IDs[i, :]
                )  # initialize a_phys
            ...
            for i in range(
                ...
                area_phys_id += np.kron(
                    self.cell_mapping_matrix[:, i],
                    np.arange(self.NN, dtype="int")[
                        i * self.N_cell[0] : (i + 1) * self.N_cell[0]
                    ],
                )
        return area_phys_id
    ...
    def calc_len_phys(self):
        ...
        if self.myBCs.isReduced:
            ...
            for i in range(self.n_topologies_cell):
                su = np.sum(
                    np.kron(
                        self.cell_mapping_matrix[:, i].T,
                        (
                            self.l_cell[i][self.myBCs.reduction_pattern_len[i, :]]
                        ).reshape((-1, 1)),
                    ),
                    axis=1,
                )
            ...
            for i in range(self.n_topologies_cell):
                su = np.sum(
                    np.kron(
                        self.cell_mapping_matrix[:, i].T, self.l_cell.reshape((-1, 1))
                    ),
                    axis=1,
                )
                g = np.append(g, su[su > 0])
...
class Layopt_IPOPT(IPOPT_Problem):
    def __init__(self, N, M, l_true, joint_cost, B, f, s_c, s_t, E):
        ...
        self.l = l_true + joint_cost * np.max(
            l_true
        )  # indipendency from the number of cells [mm]
        ...
        self.B_T_coo = (
            self.B.T.tocoo()
        )  # Horrible but necessary to conserve the very same order of dComp_dU
    ...
    def constraints(self, x):
        ...
        ### Compatibility (N eq)
        compatibility = (
            x[self.area] * self.E / self.l * (self.B.T @ x[self.U]) - x[self.force]
        )
    ...
    def jacobianstructure(self):
        ...
        row, column = (
            self.B_row,
            self.B_column + self.N,
        )  # the indexes are translated to the corresponding design variables (force)
        ### Stress (2*N eq)
    ...
    def jacobian(self, x):
        ...
        dComp_dU = (
            sp.diags(x[self.area] * self.E / self.l) @ self.B.T
        )  # Multiply the column of B.T and the main diagnonal of diag
...
class Layopt_IPOPT_Buck(IPOPT_Problem):
    def __init__(self, N, M, l_true, joint_cost, B, f, s_c, s_t, E, s_buck):
        ...
        self.s_buck = s_buck
        self.a_cr = (
            -s_c * l_true**2 / self.s_buck
        )  # Evaluation of the critical section for the members
    ...
    def constraints(self, x):
        ...
        ### Compatibility (N eq)
        compatibility = (
            x[self.area] * self.E / self.l_true * (self.B.T @ x[self.U]) - x[self.force]
        )
        ### Buckling (N eq)
    ...
    def jacobian(self, x):
        ...
        dComp_dU = (
            sp.diags(x[self.area] * self.E / self.l_true) @ self.B.T
        )  # Multiply the column of B.T and the main diagnonal of diag
        dComp_dU.sort_indices()  # Needed to match the order of B.T given in jacobianstructure
        dComp_dU = dComp_dU.tocoo()
        # Area
    ...
    def hessianstructure(self):
        ...
        # Displacements
        i, j = np.meshgrid(
            np.arange(2 * N, 2 * N + M, dtype="int"), np.arange(N, dtype="int")
        )
    ...
    def hessian(self, x, l, obj_factor):
        ...
        Hessian = sp.coo_matrix(
            (data, (i, j)), shape=(x.size, x.size)
        ).tocsc()  # check which one is better
        row, col = self.hessianstructure()
...
class Layopt_IPOPT_VL(IPOPT_Problem):
    def __init__(
        ...
        self.l = self.l_true + joint_cost * np.max(
            self.l_true
        )  # indipendency from the number of cells [mm]
        self.B = B  # Equilibrium forces matrix
        self.f = f  # External forces [N]
        ...
        self.n_topologies_cell = np.size(
            cell_mapping_matrix, 1
        )  # Number of different cell topologies

        # VL parameters
        if self.myBCs.isReduced:
            self.l_cell = self.myBCs.l_cell_full.copy() + joint_cost * np.max(
                self.l_true
            )  # change, different cell size per different reduced cell
        else:
            self.l_cell = self.l[
                : N_cell[0]
            ].copy()  # Length of every member of one cell
    ...
    def constraints(self, x):
        ...
        ### Compatibility (N eq)
        compatibility = (
            area_phys * self.E / self.l_true * (self.B.T @ x[self.U]) - x[self.force]
        )
        cons = np.concatenate((equilibrium, stress, compatibility))
        return cons
    ...
    def jacobian(self, x):
        ...
        dComp_dA = self.B.T @ x[self.U] * self.E / self.l_true  # Diagonal term, i==j
        dComp_dU = (
            sp.diags(area_phys * self.E / self.l_true) @ self.B.T
        )  # Multiply the column of B.T and the main diagnonal of diag
        dComp_dU.sort_indices()  # Needed to match the order of B.T given in jacobianstructure
        dComp_dU = dComp_dU.tocoo()
        # Area
...
class Layopt_IPOPT_VL_Buck(IPOPT_Problem):
    def __init__(
        ...
        self.B_T_coo = (
            self.B.T.tocoo()
        )  # Horrible but necessary to keep the very same order of dComp_dU
        self.B_T_row, self.B_T_column = self.B_T_coo.row, self.B_T_coo.col
    ...
    def hessianstructure(self):
        ...
        # Displacements
        temp_i, temp_j = np.meshgrid(
            np.arange(NN + N, NN + N + M, dtype="int"), np.arange(NN, dtype="int")
        )
        i = temp_i.ravel()
        j = temp_j.ravel()
    ...
    def hessian(self, x, l, obj_factor):
        ...
        # Area
        i = np.append(
            i.ravel(), np.tile(np.arange(NN, dtype="int"), self.myBCs.ncel_str)
        )
        j = np.append(
            j.ravel(), np.tile(np.arange(NN, dtype="int"), self.myBCs.ncel_str)
        )
        data = np.append(data, 2 * self.s_buck / self.l_true**2 * l[buck_id])
        # Force -
...
class Layopt_IPOPT_Buck_multiload(IPOPT_Problem):
    def __init__(self, N, M, l_true, joint_cost, B, f, dofs, s_c, s_t, E, s_buck):
        ...
        for p in range(self.n_load_cases):
            ...
            self.U.append(
                slice(
                    N + self.n_load_cases * N + (p) * M,
                    N + self.n_load_cases * N + (p + 1) * M,
                )
            )
    ...
    def constraints(self, x):
        ...
        for p in range(self.n_load_cases):
            comp = (
                x[self.area] * self.E / self.l_true * (self.B.T @ x[self.U[p]])
                - x[self.force[p]]
            )
    ...
    def jacobianstructure(self):
        ...
        for p in range(self.n_load_cases):
            row = np.append(
                row,
                range(
                    self.M * self.n_load_cases + p * self.N,
                    self.M * self.n_load_cases + (p + 1) * self.N,
                ),
            )
            column = np.append(column, np.r_[self.area])
            row = np.append(
        ...
        for p in range(self.n_load_cases):
            row = np.append(
                row,
                range(
                    self.M * self.n_load_cases
                    + self.n_load_cases * self.N
                    + p * self.N,
                    self.M * self.n_load_cases
                    + self.n_load_cases * self.N
                    + (p + 1) * self.N,
        ...
        for p in range(self.n_load_cases):
            ...
            column = np.append(
                column,
                self.B_T_column + self.N + self.N * self.n_load_cases + p * self.M,
            )  # U
        ### Buckling (N eq)
        for p in range(self.n_load_cases):
            row = np.append(
    ...
    def jacobian(self, x):
        ...
        for p in range(self.n_load_cases):
            dComp_dA = (
                self.B.T @ x[self.U[p]] * self.E / self.l_true
            )  # Diagonal term, i==j
            dComp_dU = (
                sp.diags(x[self.area] * self.E / self.l_true) @ self.B.T
            )  # Multiply the column of B.T and the main diagnonal of diag
        ...
        for p in range(self.n_load_cases):
            # Area
            Jacobian = np.append(
                Jacobian, 2 * x[self.area] * self.s_buck / self.l_true**2
            )
            # Force
            Jacobian = np.append(Jacobian, np.ones(self.N))
        return Jacobian
...
class IPOPT_Problem_Free_Form:
    ...
    def calc_area_phys_sect(self, area_optim):
        ...
        temp = np.concatenate(
            [
                np.tile(area_cell, self.myBCs.ncel_str),
                np.tile(area_sect, self.myBCs.number_section),
            ]
        )
        return temp
    ...
    def calc_area_optim(self, area_phys):
        ...
        p_cell = np.array(
            np.where(
                (self.myBCs.ground_structure[:, 2] != -1)
                & (self.myBCs.ground_structure[:, 3] == 0)
            )
        ).ravel()
    ...
    def calc_area_phys_id(self):
        ...
        temp = np.concatenate(
            [np.tile(id_area_cell, self.myBCs.ncel_str), id_area_aperiodic]
        )
        return temp
    ...
    def calc_area_phys_id_sect(self):
        ...
        temp = np.concatenate(
            [
                np.tile(id_area_cell, self.myBCs.ncel_str),
                np.tile(id_area_sect, self.myBCs.number_section),
            ]
        )
        return temp
    ...
    def calc_len_optim_sect(self):
        ...
        a = np.array(np.where(self.myBCs.ground_structure[:, 2] == -1)).ravel()
        temp = np.concatenate(
            [self.l_cell * self.myBCs.ncel_str, self.l_sec * self.myBCs.number_section]
        )
        return temp
...
class Layopt_IPOPT_VL_Buck_Free_Form(IPOPT_Problem_Free_Form):
    def __init__(self, NN, N_cell, N, M, myBCs, joint_cost, B, f, s_c, s_t, E, s_buck):
        ...
        # VL parameters
        self.l_cell = (
            self.myBCs.ground_structure_length_cell.copy()
            + joint_cost * np.max(self.l_true)
        )
    ...
    def constraints(self, x):
        ...
        compatibility = (
            area_phys * self.E / self.l_true * (self.B.T @ x[self.U]) - x[self.force]
        )  # / 1e4 # scaled because very different with respect to other entries
    ...
    def hessian(self, x, l, obj_factor):
        ...
        j, i = temp.row, temp.col + (NN + N)
        try:
            j[(j < NN_cell) & (j >= N_cell)] = (
                j % N_cell
            )  # Only the first cellule is important for sensitivity
        except:
            pass
        try:
            j[j >= NN_cell] = (
        ...
        i = np.append(
            ...
            np.concatenate(
                [
                    np.tile(np.arange(N_cell, dtype="int"), self.myBCs.ncel_str),
                    np.arange(N_cell, NN, dtype="int"),
                ]
            ),
        )
        j = np.append(
    ...
    def intermediate(
        ...
        self.it = iter_count
        self.obj_hist = np.append(self.obj_hist, self.volume_phys)
        self.constr_equilib_hist = np.append(
            self.constr_equilib_hist, self.viol_equilib
        )
        ...
        print(
            "Phys. Vol: {0:.5f},  Eq. Cons: {1:.2e},  SC. Cons: {2:.2e},  ST. Cons: {3:.2e},  Comp. Cons: {4:.2e},  Buck. Cons: {5:.2e}\n".format(
                self.obj_hist[-1],
                self.constr_equilib_hist[-1],
                self.constr_s_c_hist[-1],
                self.constr_s_t_hist[-1],
                self.constr_comp_hist[-1],
                self.constr_buck_hist[-1],
            )
        )
...
class Layopt_IPOPT_VL_Buck_Free_Form_Section(IPOPT_Problem_Free_Form):
    def __init__(
        self, NN, N_cell, N_sec, N, M, myBCs, joint_cost, B, f, s_c, s_t, E, s_buck
    ):
        # Use the class constructor to declare the variable used by the optimizer during the optimization
        self.NN = NN  # Number of members of the area design variables
        self.N_cell = N_cell  # Number of members of a single cell
        ...
        self.l_sec = (
            self.myBCs.ground_structure_length_section.copy()
            + joint_cost * np.max(self.l_true)
        )
...
class Layopt_IPOPT_Buck_Free_Form_multiload(IPOPT_Problem_Free_Form):
    ...
    def constraints(self, x):
        ...
        for p in range(self.n_load_cases):
            buck = (
                x[self.force[p]]
                + (self.s_buck / self.l_true**2) * x[self.area] ** 2 / self.sf[p]
            )
            buckling = np.append(buckling, buck)
    ...
    def jacobian(self, x):
        ...
        for p in range(self.n_load_cases):
            # Area
            Jacobian = np.append(
                Jacobian, 2 * x[self.area] * self.s_buck / self.l_true**2 / self.sf[p]
            )
            # Force
            Jacobian = np.append(Jacobian, np.ones(self.N))
    ...
    def intermediate(
        ...
        print(
            "Phys. Vol: {0:.5f},  Eq. Cons: {1:.2e},  SC. Cons: {2:.2e},  ST. Cons: {3:.2e},  Comp. Cons: {4:.2e},  Buck. Cons: {5:.2e}".format(
                self.obj_hist[-1],
                self.constr_equilib_hist[-1],
                self.constr_s_c_hist[-1],
                self.constr_s_t_hist[-1],
