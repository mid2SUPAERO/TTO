...
def calcB(myBCs: BCS.MBB2D_Symm):
    m, nA, nB = (
        len(myBCs.ground_structure),
        myBCs.ground_structure[:, 0].astype(int),
        myBCs.ground_structure[:, 1].astype(int),
    )
    l, X, Y = (
        myBCs.ground_structure_length,
        myBCs.nodes[nB, 0] - myBCs.nodes[nA, 0],
        myBCs.nodes[nB, 1] - myBCs.nodes[nA, 1],
    )
    d0, d1, d2, d3 = (
        myBCs.dofs[nA * 2],
        myBCs.dofs[nA * 2 + 1],
        myBCs.dofs[nB * 2],
        myBCs.dofs[nB * 2 + 1],
    )  # Defined on boundary conditions. One line for each candidate
...
def calcB_3D(myBCs: BCS.CantileverBeam_3D):
    ...
    l, X, Y, Z = (
        ...
        myBCs.nodes[nB, 0] - myBCs.nodes[nA, 0],
        myBCs.nodes[nB, 1] - myBCs.nodes[nA, 1],
        myBCs.nodes[nB, 2] - myBCs.nodes[nA, 2],
    )
    d0, d1, d2, d3, d4, d5 = (
        myBCs.dofs[nA * 3],
        myBCs.dofs[nA * 3 + 1],
    ...
    s = np.concatenate(
        (-X / l * d0, -Y / l * d1, -Z / l * d2, X / l * d3, Y / l * d4, Z / l * d5)
    )
    r = np.concatenate(
        (nA * 3, nA * 3 + 1, nA * 3 + 2, nB * 3, nB * 3 + 1, nB * 3 + 2)
    )  # dofs
...
def reduce_BCs(
    ...
    reduced_BCs.candidates = (
        a >= keep_value
    )  # (q>0 & a>keep_value_t) + (q<0 & a>keep_value_c)
    reduced_BCs.candidates_post_unstable_node_suppression = np.where(a >= keep_value)[
        0
    ]  # (q>0 & a>keep_value_t) + (q<0 & a>keep_value_c)
    a_reduced = a[reduced_BCs.candidates]

    # Ground structure (step 1)
    reduced_BCs.ground_structure_not_reduced = (
        BCs.ground_structure.copy()
    )  # Used for plotting
    ...
    reduced_BCs.ground_structure_length = BCs.ground_structure_length[
        reduced_BCs.candidates
    ].copy()
        ...
        if delete_chain:
            if BCs.is3D:
                ...
                unique, counts = np.unique(
                    reduced_BCs.ground_structure, return_counts=True
                )
                possible_chain_nodes = np.array(unique[np.where(counts == 2)]).ravel()
                chain_nodes = []
                for k in range(
                    possible_chain_nodes.shape[0]
                ):  # needed to avoid adding BC nodes
                    candidates_id = (
                        reduced_BCs.ground_structure[:, 0] == possible_chain_nodes[k]
                    ) | (reduced_BCs.ground_structure[:, 1] == possible_chain_nodes[k])
                    ...
                    v2 = n4 - n3
                    if np.allclose(
                        np.linalg.norm(np.cross(v2, v1), 2), 0, rtol=1e-2
                    ):  # cross prod = 0
                        if (possible_chain_nodes[k] > start) and (
                            possible_chain_nodes[k] < end
                        ):  # If not some BCs can become candidates
                            if (
                                are_bars_in_compression
                            ):  # We charge only nodes in a compression chain
                ...
                for i, node in enumerate(
                    chain_nodes
                ):  # need to use loops and not a vectorized routine
                    leng = 0
                    candidates_to_merge = reduced_BCs.ground_structure[
                        (reduced_BCs.ground_structure[:, 0] == node)
                        | (reduced_BCs.ground_structure[:, 1] == node),
                        :,
                    ]
                    merged_candidates.append(
                        sorted([candidates_to_merge[candidates_to_merge != node]])
                    )
                    ...
                    for k in range(candidates_to_merge.shape[0]):
                        dell = int(
                            np.where(
                                (
                                    reduced_BCs.ground_structure
                                    == candidates_to_merge[k, :]
                                ).all(axis=1)
                            )[0]
                        )
                        delll.append(dell)
                    ...
                    for k in range(candidates_to_merge.shape[0]):
                        ...
                        reduced_BCs.ground_structure = np.delete(
                            reduced_BCs.ground_structure, dell, axis=0
                        )
                        add_a = a_reduced[dell]
                        add_q = q_reduced[dell]
                        a_reduced = np.delete(a_reduced, dell, axis=0)
                        id_cand = np.nonzero(
                            reduced_BCs.candidates_post_unstable_node_suppression
                        )[0]
                        ...
                        reduced_BCs.ground_structure_length = np.delete(
                            reduced_BCs.ground_structure_length, dell, axis=0
                        )
                    add = np.array(merged_candidates[i]).reshape(1, 2)
                    if (
                        not np.equal(add, reduced_BCs.ground_structure)
                        .all(axis=1)
                        .any()
                    ):  # check before adding duplicates
                        reduced_BCs.ground_structure = np.vstack(
                            [reduced_BCs.ground_structure, add]
                        )
                        ...
                        reduced_BCs.ground_structure_length = np.append(
                            reduced_BCs.ground_structure_length, leng
                        )
                ...
                for k in range(
                    ...
                    cand = reduced_BCs.ground_structure[
                        (reduced_BCs.ground_structure[:, 0] == possible_chain_nodes[k])
                        | (
                            reduced_BCs.ground_structure[:, 1]
                            == possible_chain_nodes[k]
                        ),
                        :,
                    ]
                ...
                for i, node in enumerate(
                    ...
                    for k in range(candidates_to_merge.shape[0]):
                        ...
                        try:
                            ...
                            reduced_BCs.candidates_post_unstable_node_suppression = np.delete(
                                reduced_BCs.candidates_post_unstable_node_suppression, r
                            )
                            delete = True
                        except:
                            delete = False
                    ...
                    if (
                        ...
                        a_reduced = np.append(a_reduced, add_a)
                        if delete:
                            reduced_BCs.candidates_post_unstable_node_suppression = np.append(
                                reduced_BCs.candidates_post_unstable_node_suppression,
                                eq,
                            )
                        try:
    ...
    if BCs.is3D:
        ...
        reduced_BCs.dofs_candidates = np.append(
            reduced_BCs.dofs_candidates * 3, reduced_BCs.nodes_candidates * 3 + 1
        )
        reduced_BCs.dofs_candidates = np.sort(
            np.append(reduced_BCs.dofs_candidates, reduced_BCs.nodes_candidates * 3 + 2)
        )
    else:
        reduced_BCs.dofs_candidates = reduced_BCs.nodes_candidates.copy()
        reduced_BCs.dofs_candidates = np.sort(
            np.append(
                reduced_BCs.dofs_candidates * 2, reduced_BCs.nodes_candidates * 2 + 1
            )
        )
    ...
    replace_matrix = np.vstack(
        [reduced_BCs.nodes_candidates, np.arange(reduced_BCs.nodes_candidates.size)]
    ).T  # find (left col) and replace (right col) matrix used for the select conditions
    ...
    if cell_mapping_vector is not None:
    ...
    if a_cell is not None:
        ...
        for i in range(a_cell_reduced.size):
            a_cell_reduced_pad[i] = np.pad(
                a_cell_reduced_pad[i],
                pad_width=(0, reduced_BCs.N_cell_max - a_cell_reduced_pad[i].size),
                constant_values=-1,
            )

        # Create a bool matrix used to understand which sections are active
        reduced_BCs.reduction_pattern = (a_cell_reduced_pad[0] >= 0).reshape(
            1, -1
        )  # this array should be at least 2D
        for i in range(a_cell_reduced.size - 1):
            reduced_BCs.reduction_pattern = np.vstack(
                (reduced_BCs.reduction_pattern, a_cell_reduced_pad[i + 1] >= 0)
            )
        ...
        for i in range(a_cell_reduced.size - 1):
            reduced_BCs.reduction_pattern_len = np.vstack(
                (reduced_BCs.reduction_pattern_len, a_cell[i + 1] > keep_value)
            )

        # Create a matrix with the unreduced legnht of the members of every cell topology
        reduced_BCs.l_cell_full = np.tile(
            BCs.ground_structure_length[: a_cell[0].shape[0]], a_cell.shape[0]
        ).reshape((-1, a_cell[0].shape[0]))

        # Update the list of the ground structure of the reduced problem
        reduced_BCs.ground_stucture_list_cell = []
        for i in range(n_topologies_cell):
            reduced_BCs.ground_stucture_list_cell.append(
                np.array(BCs.ground_stucture_list_cell)[a_cell[i] > keep_value].tolist()
            )

    reduced_BCs.ground_stucture_list_not_reduced = BCs.ground_structure[
        :, [0, 1]
    ].tolist()
    ...
    if reduced_BCs.isFreeForm:
        ...
        reduced_BCs.ground_structure_cellular = reduced_BCs.ground_structure[
            p, :
        ].copy()
        reduced_BCs.ground_structure_aperiodic = reduced_BCs.ground_structure[
            a, :
        ].copy()

        reduced_BCs.N = len(
            reduced_BCs.ground_structure
        )  # Number of member of the Ground Structure
        ...
        if a_cell is not None:
            reduced_BCs.ground_structure_length_cell = BCs.ground_structure_length_cell[
                keep_cell
            ].copy()
...
def mapping_matrix_init(cell_mapping_vector, BC):
    ...
    for i, item in enumerate(topologies_ID):  # starting from the lower left corner
        cell_mapping_matrix[np.where(cell_mapping_vector == item), i] += (
            1  # create matrix to correctly assign the different cell topologies according to cell_mapping_matrix
        )
    ...
    NN = np.sum(
        BC.N_cell
    )  # Number of elements in the cell times the number of different cells (area design variables)

    return NN, n_topologies_cell, cell_mapping_matrix


def calculate_starting_point_on_unreduced_BCs(
    myBCs, myBCs_reduced, N_design_var, area_id, force_id, U_id, E, a_init, a_fem
):
...
def calculate_starting_point_cell_on_unreduced_BCs(
    myBCs,
    myBCs_reduced,
    N_design_var,
    area_id,
    ...
    a_cell_list = []
    for i in range(n_topologies_cell):
        a_cell_list = np.append(
            a_cell_list, a_cell[i][a_cell[i] > -1e-5]
        )  # Eliminate -1 values used for the Kronecher product, a little neg value is accepted for numeric errors
        a_cell
    ...
    for i in range(n_topologies_cell):  # starting from the lower left corner
        ...
        a_cell_pad[myBCs_reduced.keep_cell[i]] = a_cell[i][
            range(myBCs_reduced.N_cell[i])
        ]
...
def save_files(
    ...
    if LP:
        ...
        np.savetxt(
            folder + "GS_LP_cand.dat", myBCs.ground_structure[candidates], fmt="%d"
        )
    ...
    if is3D:
        if LP:
            with open(foldername + "/" + "log_LP" + ".txt", "w") as f:
                if system == 0:
                    f.write(
                        "Vol: {0:.2f} mm3, Weight: {1:.5f} kg\n".format(
                            vol, vol * rho * 1000
                        )
                    )
                elif system == 1:
                    f.write(
                        "Vol: {0:.2f} m3, Weight: {1:.5f} kg\n".format(vol, vol * rho)
                    )
                ...
                f.write(
                    "Min slenderness ratio: {0:.3f}\n".format(
                        np.min(
                            myBCs.ground_structure_length[candidates]
                            / np.sqrt((s_buck * a[candidates]) / (np.pi**2 * E))
                        )
                    )
                )  # ratio between length and Radius of gyration
                if is_free_form:
                    a_aper = a[myBCs.ground_structure[:, 2] == -1]
                    vol_aper = (
                        a_aper.T
                        @ myBCs.ground_structure_length[
                            myBCs.ground_structure[:, 2] == -1
                        ]
                    )
                    if vol_aper.size == 0:
                        vol_aper = 0
                    ...
                    if system == 0:
                        f.write(
                            "Vol_aper: {0:.2f} mm3, Weight_aper: {1:.5f} kg\n".format(
                                vol_aper, vol_aper * rho * 1000
                            )
                        )
                        f.write(
                            "Vol_cell: {0:.2f} mm3, Weight_cell: {1:.5f} kg\n".format(
                                vol_cell, vol_cell * rho * 1000
                            )
                        )
                        f.write(
                            "Vol_sec: {0:.2f} mm3, Weight_sec: {1:.5f} kg\n".format(
                                vol_sect, vol_sect * rho * 1000
                            )
                        )
                        f.write(
                            "Vol_equal: {0:.2f} mm3, Weight_equal: {1:.5f} kg\n".format(
                                vol_equal, vol_equal * rho * 1000
                            )
                        )
                    elif system == 1:
                        f.write(
                            "Vol_aper: {0:.2f} m3, Weight_aper: {1:.5f} kg\n".format(
                                vol_aper, vol_aper * rho
                            )
                        )
                        f.write(
                            "Vol_cell: {0:.2f} m3, Weight_cell: {1:.5f} kg\n".format(
                                vol_cell, vol_cell * rho
                            )
                        )
                        f.write(
                            "Vol_sec: {0:.2f} m3, Weight_sec: {1:.5f} kg\n".format(
                                vol_sect, vol_sect * rho
                            )
                        )
                        f.write(
                            "Vol_equal: {0:.2f} m3, Weight_equal: {1:.5f} kg\n".format(
                                vol_equal, vol_equal * rho
                            )
                        )
        else:
            with open(foldername + "/" + "log" + ".txt", "w") as f:
                ...
                f.write(
                    "Min slenderness ratio: {0:.3f}\n".format(
                        np.min(
                            myBCs.ground_structure_length[candidates]
                            / np.sqrt((s_buck * np.abs(a[candidates])) / (np.pi**2 * E))
                        )
                    )
                )  # ratio between length and Radius of gyration
                if is_free_form:
                ...
                if vol_LP != False:
                    f.write(
                        "NLP volume add: {0:.6f} %".format(vol / vol_LP * 100 - 100)
                    )
    else:
        if LP:
            with open(foldername + "/" + "log_LP" + ".txt", "w") as f:
                ...
                f.write(
                    "Vol_star: {0:.5f} PL/sigma\n".format(
                        vol
                        / (
                            ff
                            * max(L)
                            / ((stress_tension_max - stress_compression_max) / 2)
                        )
                    )
                )
                if system == 0:
...
#     vol = prob.solve(verbose=True, ) # Problem solving
...
def solveLP_2D_SLP_Buckling(
    myBCs: BCS.MBB2D_Symm,
    stress_T_max,
    stress_C_max,
    s_buck,
    joint_cost=0,
    cellsEquals: bool = False,
    cell_mapping_vector=False,
    a_init=False,
    chain=True,
    foldername=False,
):
    l = myBCs.ground_structure_length + joint_cost * np.max(
        myBCs.ground_structure_length
    )  # indipendency from the number of cells
    B = calcB(myBCs)  # equilibrium matrix
    #
    # Design variable initialization
    #
    # VL parameter
    NN, n_topologies_cell, cell_mapping_matrix = mapping_matrix_init(
        cell_mapping_vector, myBCs
    )
    ...
    if cellsEquals:
        ...
        a_cell = np.zeros(n_topologies_cell, dtype="object_")
        for i in range(n_topologies_cell):  # starting from the lower left corner
            a_cell[i] = cvx.Variable(
                (int(myBCs.ground_structure.shape[0] / myBCs.ncel_str), 1), nonneg=True
            )
            a_wholestruct = cvx.kron(
                cell_mapping_matrix[:, i].reshape(-1, 1), a_cell[i]
            )  # initialize a_phys
    else:
        a_cell = cvx.Variable(
            (myBCs.ground_structure.shape[0], 1), nonneg=True
    ...
    while (tol > 1e-06 and i < 300) or reset:
        ...
        a_0_buck = (
            a.copy()
        )  # need for another variable. we cant use a_buc for the tolerance calc
        buckling_length = myBCs.ground_structure_length.copy()
        ...
        if (tol < 1e-4 and tol_old < 1e-4 and reset) or (i == 75) or (i == 150):
            ...
            if plot_intermediate_reset:
                ...
                folder_case = (
                    foldername
                    + "/LP/reset/"
                    + "It_{0:0>4}-K_{2:0>3}-val_{1:.3f}/".format(
                        i, reset_val_old, k_buck
                    )
                )
                if not os.path.isdir(folder_case):
                    os.makedirs(folder_case)
                trussplot.plot2D.plotTruss(
                    myBCs,
                    a,
                    q,
                    max(a) * 1e-3,
                    myBCs.ground_structure_length.T @ a,
                    folder_case,
                )
            ...
            if k_buck >= 16:  # stopping condition
                reset = False
            print("REINIT")
        ...
        if chain_control and i != 0:
            ...
            for k in range(
                ...
                candidates_id = (
                    reduced_ground_structure[:, 0] == possible_chain_nodes[k]
                ) | (reduced_ground_structure[:, 1] == possible_chain_nodes[k])
            ...
            if len(chain_nodes) != 0:  # augmented length approach
                ...
                for k, node in enumerate(
                    ...
                    candidates_bars_id = (reduced_ground_structure[:, 0] == node) | (
                        reduced_ground_structure[:, 1] == node
                    )
                    ...
                    if k != 0:
                        if node in treated_nodes[-1]:
                            ...
                            last = (
                                candidates_bars_id & ~member_to_add
                            )  # get only the new bar and update the length based on it
                            ...
                            total_leng = np.sum(
                                buckling_length_reduced[candidates_bars_id]
                            )
                            member_to_add = candidates_bars_id
                ...
                if only_first:
                    ...
                    first_bar_chain_pattern = np.zeros(
                        buckling_length_reduced.size, dtype="bool"
                    )
                    first_bar_chain_pattern[first_bar_chain] = True
                    buckling_length_reduced[~first_bar_chain_pattern] = (
                        myBCs.ground_structure_length[
                            reduction_pattern
                        ][~first_bar_chain_pattern]
                    )
        ...
        if only_first_activate_constraint:
            for ii, leading in enumerate(first_bar_chain):
                for kk, other in enumerate(tuple(member_to_add_id[ii])):
                    if leading != other:
                        side_constr = (
                            a_wholestruct[reduction_pattern][leading]
                            <= a_wholestruct[reduction_pattern][other]
                        )
        ...
        try:
            ...
            cons_buck = q >= cvx.atoms.affine.binary_operators.multiply(
                xx, b
            )  # Linearized buckling constr
        ...
        )  # Problem solving bfs=True for mosek if you want the basic solution (simplex)
        q = np.array(q.value).ravel()  # Optimized forces
...
#     obj_hist = nlp._Problem__objective.__self__.obj_hist
...
def solveNLP_2D_IPOPT_Buckling(
    myBCs: BCS.MBB2D_Symm,
    myBCs_unred: BCS.MBB2D_Symm,
    stress_T_max,
    stress_C_max,
    E,
    s_buck,
    ...
    ## Starting point
    x0 = calculate_starting_point_on_unreduced_BCs(
        myBCs_unred, myBCs, N_design_var, area_id, force_id, U_id, E, a_init, a_fem
    )
    ...
    nlp = cyipopt.Problem(
        n=len(x0),
        m=len(cl),
        problem_obj=ipopt_routines.Layopt_IPOPT_Buck(
            N,
            M,
            myBCs.ground_structure_length,
            joint_cost,
...
def solveNLP_2D_IPOPT_VL_Buckling(
    myBCs: BCS.MBB2D_Symm,
    myBCs_unred: BCS.MBB2D_Symm,
    stress_T_max,
    stress_C_max,
    ...
    ## Starting point
    x0 = calculate_starting_point_cell_on_unreduced_BCs(
        myBCs_unred,
        myBCs,
        N_design_var,
        area_id,
    ...
    NLP_prob = ipopt_routines.Layopt_IPOPT_VL_Buck(
        NN,
        N_cell,
        N,
        M,
...
def solveLP_3D_SLP_Buckling(
    ...
    if cellsEquals:
        ...
        for i in range(n_topologies_cell):  # starting from the lower left corner
            a_cell[i] = cvx.Variable(
                (int(myBCs.ground_structure.shape[0] / myBCs.ncel_str), 1)
            )
            a_wholestruct = a_wholestruct + cvx.kron(
                cell_mapping_matrix[:, i].reshape(-1, 1), a_cell[i]
            )  # initialize a_phys
    else:
        a_cell = (
            cvx.Variable((myBCs.ground_structure.shape[0], 1)) * 0.1
        )  # initialization of the design variables
        a_wholestruct = a_cell
    obj = cvx.Minimize(l.T @ a_wholestruct)  # definition of the objective function
    #
    ...
    while (tol > 1e-06 and i < 300) or reset:
        ...
        a_0_buck = (
            a.copy()
        )  # need for another variable. we cant use a_buck for the tolerance calc
        ...
        if not cellsEquals:
            if chain_control and i != 0:
                # Identify if there is some chain buckling
                reduction_pattern = a_0 > (1e-3 * max(a_0))
                reduced_ground_structure = myBCs.ground_structure[
                    reduction_pattern
                ].copy()
                reduced_q = q[reduction_pattern].copy()
                unique, counts = np.unique(reduced_ground_structure, return_counts=True)
                ...
                if len(chain_nodes) != 0:  # augmented length approach
                    ...
                    for k, node in enumerate(
                        ...
                        candidates_bars_id = (
                            reduced_ground_structure[:, 0] == node
                        ) | (reduced_ground_structure[:, 1] == node)
                        candidates_bars = reduced_ground_structure[
                            candidates_bars_id, :
                        ]
                        # check if the 2 members have a nodes in common with the last treated chain_node, if not leng = 0
                        if k != 0:
                            if node in treated_nodes[-1]:
                                treated_nodes[-1].update(
                                    candidates_bars.ravel().tolist()
                                )
                                last = (
                                    candidates_bars_id & ~member_to_add
                                )  # get only the new bar and update the length based on it
                                member_to_add += candidates_bars_id
                                total_leng += buckling_length_reduced[last]
                                member_to_add_id[-1].update(
                                    set(np.where(member_to_add)[0])
                                )
                            else:
                                first_bar_chain.append(
                                    np.where(candidates_bars_id)[0][0]
                                )
                                treated_nodes.append(
                                    set(candidates_bars.ravel().tolist())
                                )
                                total_leng = np.sum(
                                    buckling_length_reduced[candidates_bars_id]
                                )
...
def solveNLP_3D_IPOPT_VL_Buckling(
    myBCs: BCS.Wing_3D,
    myBCs_unred: BCS.Wing_3D,
    stress_T_max,
    stress_C_max,
    E,
    s_buck,
...
#         #
#         prob = cvx.Problem(obj, cons) # Creation of LP
#         vol = prob.solve(verbose=False, max_iter = 2000, reltol = 5.0e-5, reltol_inacc = 1.0e-2)
#         q = np.array(q.value).ravel() # Optimized forces
#         a_1 = np.array(a_wholestruct.value).ravel() # Optimized areas
#         a = a_1.copy()
#         tol = np.linalg.norm(a-a_0, np.inf)
...
#     obj_hist = nlp._Problem__objective.__self__.obj_hist
#     eq_hist = nlp._Problem__objective.__self__.constr_equilib_hist
#     s_c_hist = nlp._Problem__objective.__self__.constr_s_c_hist
#     s_t_hist = nlp._Problem__objective.__self__.constr_s_t_hist
#     comp_hist = nlp._Problem__objective.__self__.constr_comp_hist
#     buck_hist = nlp._Problem__objective.__self__.constr_buck_hist
...
#         #
#         prob = cvx.Problem(obj, cons) # Creation of LP
#         vol = prob.solve(verbose=False, max_iter = 5000) # Problem solving bfs=True for mosek if you want the basic solution (simplex)
#         q = np.array([np.array(qi.value).ravel() for qi in q]).T
#         a_1 = np.array(a_wholestruct.value).ravel() # Optimized areas
#         a = a_1.copy()
#         tol_old = tol
...
#         #
#         prob = cvx.Problem(obj, cons) # Creation of LP
#         vol = prob.solve(verbose=False, max_iter = 5000)#, reltol = 5.0e-3, feastol = 1.0e-4, reltol_inacc = 1.0e-2, feastol_inacc = 1.0e-2)
#         q = np.array([np.array(qi.value).ravel() for qi in q]).T
#         a_1 = np.array(a_wholestruct.value).ravel() # Optimized areas
#         a = a_1.copy()
#         tol_old = tol
...
if __name__ == "__main__":
    ...
    myBCs = BCS.MichCantilever_Tugilimana(
        nnodx_cel, nnody_cel, nelx_str, nely_str, f, L
    )
    ...
    # Save results
    if not cellsEquals:
        foldername = myBCs.name + "-dim{0}x{1}-{2}x{3}-{4}x{5}-JC={6}-nocell".format(
    ...
    t = time.time()
    vol_LP, a, q, U, obj_hist_LP = solveLP_2D(
        myBCs, stress_tension_max, stress_compression_max, joint_cost
    )
    elapsed_LP = time.time() - t
    trussplot.plot2D.plotRoutine(
        myBCs,
        a,
        q,
        U,
